...
const AddOn = ({ addOn, ctaText }) => {
  const { display_name__limio, description__limio, price__limio } =
  ...
  const offer =
    basketItems && basketItems.length > 0 ? basketItems[0].offer : null;
  const offerGroup = offer && offer.data.attributes.group__limio;
  const addOnGroup = R.pathOr(
    null,
    ["data", "attributes", "term_group"],
    addOn
  );
  const basketItemAddOnsIds = R.pathOr([], ["0", "addOns"], basketItems).map(
    (item) => item.addOn.id
  );
  ...
  return (
    <div className="add-on">
        ...
        <span
          ...
          dangerouslySetInnerHTML={{
            __html: sanitizeString(
              formatCurrency(
                price__limio[0].value,
                price__limio[0].currencyCode
              )
            ),
          }}
        ></span>
      ...
      <p
        ...
        dangerouslySetInnerHTML={{ __html: description__limio }}
      ></p>
