...
const Offer = ({ offer, showImage, offerWidth, primaryColor, freeTrialLink, bestValueColor }) => {
  ...
  return (
    <>
      ...
      >
        <div className={`color-bar ${best_value__limio ? "best-value-text-box" : null}`} style={{ backgroundColor: offer_card_color }}>
          {best_value__limio && bestValueText}
        </div>
        ...
        <div className="flex justify-center items-baseline my-4">
          <span
            className="mr-2 text-3xl font-extrabold"
        ...
        <p className="font-light text-gray-500 sm:text-lg dark:text-gray-400 mb-4 v">{display_equivalent_price}</p>
        <p
          className="font-light text-gray-500 sm:text-lg dark:text-gray-400 mb-6 "
          dangerouslySetInnerHTML={{
            __html: sanitizeString(
        ...
        <span className="text-gray-400 text-sm text-center" dangerouslySetInnerHTML={{ __html: sanitizeString(freeTrialLink) }} />
      </div>
    </>
  )
}
