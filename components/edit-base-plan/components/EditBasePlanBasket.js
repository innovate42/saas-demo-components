  ...
  if (!R.isEmpty(matchedAddOnsToReadd)) {
    addAddOns = R.uniqBy(R.prop("id"), matchingAddOns).map(addOn => {
      ...
      if (addOnId.startsWith("add_on")) {
        // split add on string so its not add_on-342014882310
...
function EditBasePlanBasket({
  ...
  const getPreview = () => {
    const matchingAddOns = matchAddOnsWithEntitlements(subscription.addOns, entitlementsToRemove)
    const order = buildOrder(subscription, selectedOfferObj, dateOfEffect, matchingAddOns, currentOffer, [], quantity)
