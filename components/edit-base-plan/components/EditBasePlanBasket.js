...
function EditBasePlanBasket({
  ...
  const { payment_methods, revalidate: revalidatePayments } =
    useLimioUserSubscriptionPaymentMethods(subscription.id);
  ...
  const getPreview = () => {
    // we dont really need to handle add ons here as the ui only wants the refund of the old offer and the charge of the new offer
    const order = buildOrder(
      subscription,
      selectedOfferObj,
      dateOfEffect,
  ...
  const showPaymentDetails = () => {
    ...
    const paymentMethodData = R.path(
      ["data", paymentMethodType, "result"],
      activePaymentMethod
    );
    const { CreditCardMaskNumber: creditCardMask, CreditCardType = "" } =
      paymentMethodData;
  ...
  return (
    <div className={"right-side"}>
      ...
      <div className={"flex space-between mr-4 mt-4"}>
        <div
          className={"less-bold"}
          dangerouslySetInnerHTML={{ __html: toPayText }}
        />
        <p>
      ...
      <section
        ...
        dangerouslySetInnerHTML={{ __html: longTexts }}
      ></section>
    </div>
  );
}
