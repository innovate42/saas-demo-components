...
import { checkActiveSubscriptionOffer, emptyOrNil, formatCurrency, formatDate, matchPartialString, planChangeStatus, stripPathToProductName } from "./helpers"
...
function EditBasePlanBasket({ selectedOffer, quantity }: Props): React.Node {
  ...
  const ownedAddOns = subscriptions[0].addOns.filter(addOn => addOn.status === "active" && !isExpired(addOn))
  ...
  const matchAddOnsWithEntitlements = (addOns, entitlements) => {
    ...
    addOns.forEach(addOn => {
      ...
      if (addOnEntitlements.some(entitlement => entitlements.includes(entitlement))) {
  ...
  const handleSubmit = async () => {
    ...
    const matchedRemovals = matchAddOnsWithEntitlements(ownedAddOns, entitlementsToRemove)
    const selectedOfferBillingPlan = selectedOfferObj.data.attributes.billing_plan[0]
    const matchedReAdds = matchAddOnsWithNotEntitlements(ownedAddOns, entitlementsToRemove)
    ...
    const matchedAddOnsToReadd = addOns.filter(
      addOn => matchedReAddProduct.includes(addOn.data.products[0].path) && selectedOfferBillingPlan === addOn.data.attributes.billing_option[0]
    )
  ...
  const getPreview = () => {
    const matchingAddOns = matchAddOnsWithEntitlements(subscriptions[0].addOns, entitlementsToRemove)
    const order = buildOrder(subscriptions[0], selectedOfferObj, dateOfEffect, matchingAddOns, currentOffer, [], quantity)
  ...
  return (
    <div className={"right-side"}>
      ...
      <PlanAndPricing selectedOfferObj={selectedOfferObj} price={price} processToday={processToday} />
      ...
      <div className={"flex space-between mr-4 mt-4"}>
        ...
        <p>
          {processToday.current ? !emptyOrNil(price.add) && !emptyOrNil(price.remove) ? calculateTotal(price.add, price.remove) : <LoadingSpinner /> : "--"}
        </p>
      </div>
