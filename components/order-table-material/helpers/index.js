...
export function checkPreviousSchedule(schedules) {
  ...
  const activeSchedules = schedules.filter(schedule =>
      ["active", "pending", "pending-external"].includes(schedule.status)
  )

  const currentDate = DateTime.utc().toISO()

  const prevSchedules = activeSchedules.filter(schedule => {
    const isBeforeNow = schedule.data.date <= currentDate
    const amount = parseFloat(schedule.data.amount)
    const isPositiveAmount = !isNaN(amount) && amount >= 0
    return isBeforeNow && isPositiveAmount
  })

  const sortedSchedules = prevSchedules.sort(
