...
function EditAddOnsBasket({
  ...
  React.useEffect(() => {
    ...
    if (zuoraPreview) {
      if (previewSchedule[0]?.lineItems?.length) {
        const currentSchedule = previewSchedule.find(
          s => new Date(s.date).getDay() === new Date().getDay(),
        );
  ...
  const orderTotal = () => {
    ...
    const total = matchedAddOns.reduce((acc, curr) => {
      const priceMatch = matchAddOnSchedule(
        stripPathToProductName(curr.data.products[0].path),
        true,
      );
      if (priceMatch) {
        ...
        <div>
          <Alert color={"primary"}>
            ...
            <p className={"inline pl-2"}>
              Add-ons purchased during your current billing cycle will be adjusted on a prorated
              basis.
            </p>
          </Alert>
        </div>
      ...
      {!addEmpty ? (
        <>
          ...
          <Alert color={"danger"} className={"mr-4"}>
            ...
            <p className={"alert-color pl-2"}>
              Starting on {dateOfEffect}, you will be charged the full amount for the selected
              add-on.
            </p>
          </Alert>
      ...
      {!removeEmpty ? (
        <>
          ...
          <Alert color={"danger"} className={"mr-4"}>
            <FontAwesomeIcon icon={faTriangleExclamation} />
            <p className={"alert-color pl-2"}>
              Your removed add-ons will take affect on {dateOfEffect}
            </p>
          </Alert>
          <div className="row-border" />
        </>
