...
function CancelRadioButtons({
  ...
  const onCancel = async () => {
    const option = new URLSearchParams(window.location.search).get("ltm_option")
    ...
    await sendOrder(cancelIntentOrder)
              ...
              {reasons.map((r, i) => {
                ...
                return (
                  <div key={r.value} className={"reasonListItem"}>
                    <Input type="radio" id={id} value={r.value} checked={selectedReason === r} onChange={() => setSelectedReason(r)} className={"radioButton"}/>
